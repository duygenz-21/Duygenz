...
async function restoreSystemState() {
    ...
    // 2. Khôi phục Chat History (Hạn 3 ngày)
    ...
    if (savedChat) {
        ...
        if (diffDays > 3) {
            ...
            currentSessionId = savedChat.id || currentSessionId;
...
function addLicenseUI() {
    ...
    // Note: HTML file already has license section hardcoded, so this might be redundant but safe to keep.
}
...
async function sendMessage() {
    ...
    // 2. Nếu chia hết cho 5 (5, 10, 15...) thì thả "điệp viên" đi check
    if (messageCounter > 0 && messageCounter % 5 === 0) {      
