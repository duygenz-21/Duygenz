/**
 * ==========================================================================================
 * 🛠️ MODULE 1: CONFIGURATION & CONSTANTS (CẤU HÌNH HỆ THỐNG)
 * ==========================================================================================
 * Chứa tất cả các cài đặt tĩnh, Database config và Tài nguyên mạng.
 */
...
// ⚠️ QUAN TRỌNG: Đưa DB_CONFIG lên đầu để tránh lỗi ReferenceError
const DB_CONFIG = {
    NAME: 'UltimateAIChatDB',
    VERSION: 1,
    STORES: {
        CHAT: 'chat_history',      // Lưu tin nhắn & HTML
        LICENSE: 'user_license',   // Lưu Key bản quyền
        USAGE: 'usage_tracking'    // Lưu số lượt dùng Free
    }
};
...
/**
 * ==========================================================================================
 * 💾 MODULE 2: DATABASE HELPERS (INDEXEDDB)
 * ==========================================================================================
 * Các hàm xử lý lưu trữ, phải được khai báo trước khi initChat chạy.
 */
...
/**
 * ==========================================================================================
 * 🌍 MODULE 3: GLOBAL STATE & VARIABLES
 * ==========================================================================================
...
/**
 * ==========================================================================================
 * 🔐 MODULE 5: LICENSE & PERMISSION LOGIC
 * ==========================================================================================
 */
...
async function restoreSystemState() {
    ...
    // 2. Khôi phục Chat History
    const savedChat = await dbGet(DB_CONFIG.STORES.CHAT, 'current_session');

    if (savedChat) {
        const now = new Date().getTime();
        const diffDays = (now - savedChat.lastActive) / (1000 * 60 * 60 * 24);
        ...
        if (diffDays > 3) {
            ...
            // Cập nhật lại ID phiên làm việc cũ để tiếp tục lưu vào đó
            currentSessionId = savedChat.id; 
...
async function sendMessage() {
    ...
    if (pendingVisionImages.length > 0) {
        ...
        try {
           ...
           Nhiệm vụ: Hãy viết một câu lệnh (Prompt) thật cụ thể và trước câu hỏi nhớ thêm "hãy phân tích hình ảnh " để AI Vision hiểu, trả về nội dung câu lệnh bằng tiếng Anh.
...
// ⚠️ GỌI HÀM INIT SAU KHI ĐÃ KHAI BÁO TẤT CẢ
initChat();
