...
function addLicenseUI() {
  ...
  const licenseHTML = `
    <div class="settings-section" style="border-top: 1px solid #334155; margin-top: 15px; padding-top: 15px;">
      ...
      <input type="text" 
             ...
             class="w-full p-2 rounded border border-slate-600 bg-slate-800 text-white mb-2"
             style="width: 100%; box-sizing: border-box;"
             value="${localStorage.getItem('license_key') || ''}">
      
      <div class="flex gap-2" style="display: flex; gap: 10px;">
        <button onclick="activateLicense()" class="btn-primary flex-1" style="flex: 1; padding: 8px; background: #22c55e; border:none; border-radius: 4px; color:white; cursor:pointer;">
          <i class="fas fa-check"></i> Kích hoạt
        </button>
        <button onclick="deactivateLicense()" class="btn-secondary flex-1" style="flex: 1; padding: 8px; background: #ef4444; border:none; border-radius: 4px; color:white; cursor:pointer;">
      ...
      <div class="mt-3 text-xs text-slate-400" style="margin-top: 10px; font-size: 0.75rem; color: #94a3b8;">
        ...
        <a href="#" class="text-blue-400 underline">Mua license tại đây</a>
      </div>
    </div>
  `;
  
  // Tìm vị trí chèn: Ưu tiên sau modelSettings, nếu không thì append vào cuối content
  ...
  if (modelSection && modelSection.parentNode) {
    ...
    // Nếu không tìm thấy ID modelSettings, chèn vào cuối modal
...
async function activateLicense() {
  ...
  if (result.valid) {
    ...
    // Reset Usage khi lên Pro (Optional)
    // Object.keys(usageData).forEach(k => { if(k.startsWith('free')) localStorage.setItem(k, '0'); });
