...
/**
 * ==========================================================================================
 * 11. AI CODE STUDIO (INTEGRATED IDE LOGIC)
 * ==========================================================================================
 * Tính năng: Viết code JS/Python trực tiếp, chạy ngay trên trình duyệt, không cần Backend.
 */
...
// Hàm mở IDE (Lazy Load tài nguyên)
async function openInternalIDE() {
    const modal = document.getElementById('ideModal');
    
    // 1. Nếu chưa tải thư viện CodeMirror -> Tải ngay
    if (!ideState.loaded) {
        // Hiển thị loading tạm thời
        modal.classList.remove('hidden');
        document.getElementById('ideSourceCode').value = "// ⏳ Đang tải thư viện Editor...";
        ...
        // Load JS (Load tuần tự để tránh lỗi)
        await loadScript('cm-core', 'https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.65.16/codemirror.min.js');
        await loadScript('cm-js', 'https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.65.16/mode/javascript/javascript.min.js');
        await loadScript('cm-py', 'https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.65.16/mode/python/python.min.js');
        
        ideState.loaded = true;
    }

    // 2. Hiển thị Modal
    modal.classList.remove('hidden');

    // 3. Khởi tạo Editor nếu chưa có
    if (!ideState.editor) {
        // Xóa text loading cũ
        document.getElementById('ideSourceCode').value = "// Chào sếp! Viết code ở đây nhé.\nconsole.log('Hello AI Streaming!');";
        
        ideState.editor = CodeMirror.fromTextArea(document.getElementById("ideSourceCode"), {
            lineNumbers: true,
            mode: "javascript",
            theme: "dracula",
            lineWrapping: true,
        ...
        // Fix lỗi hiển thị khi mới init (phải refresh layout)
        setTimeout(() => ideState.editor.refresh(), 100);
    }
    
    // Tự động chỉnh mode theo select box
    document.getElementById('ideLangSelect').addEventListener('change', function() {
        const mode = this.value === 'python' ? 'python' : 'javascript';
        ideState.editor.setOption('mode', mode);
        
        // Gợi ý code mẫu
        if(this.value === 'python') {
            if(ideState.editor.getValue().includes('console.log')) {
                ideState.editor.setValue("# Python Code\nimport sys\nprint('Hello from Pyodide!')\n");
            }
        }
    });
}
...
async function runIDECode() {
    ...
    consoleDiv.innerHTML = '<div class="text-slate-500">--- Running ---</div>';
    ...
    if (lang === 'javascript') {
        ...
        try {
            ...
            if (result !== undefined) logToUI(`➤ Return: ${result}`, 'result');
            
            // Trả lại console
            console.log = originalLog;
        } catch (e) {
            logToUI(`❌ Error: ${e.message}`, 'error');
        }

    } else if (lang === 'python') {
        // --- CHẠY PYTHON (PYODIDE) ---
        logToUI("🐍 Đang gọi Python Engine...", 'info');
        
        try {
            // 1. Kiểm tra Pyodide đã load chưa (tận dụng biến global của sếp)
            if (!window.pyodideObj) {
                logToUI("⏳ Đang tải Pyodide lần đầu (sẽ hơi lâu)...", 'warn');
                await loadScript('pyodide-script', RESOURCES.pyodide); // Dùng RESOURCES của sếp
                window.pyodideObj = await loadPyodide({
                    indexURL: "https://cdn.jsdelivr.net/pyodide/v0.26.2/full/"
                });
                // Load thư viện phổ biến sẵn
                await window.pyodideObj.loadPackage(["numpy", "pandas"]);
            }

            // 2. Chuyển hướng stdout của Python
            window.pyodideObj.runPython(`
                import sys
                import io
                sys.stdout = io.StringIO()
            ...
            // 3. Chạy code User
            await window.pyodideObj.runPythonAsync(code);

            // 4. Lấy kết quả stdout
            const stdout = window.pyodideObj.runPython("sys.stdout.getvalue()");
            const stderr = window.pyodideObj.runPython("sys.stderr.getvalue()");

            if (stdout) logToUI(stdout);
            if (stderr) logToUI(stderr, 'error');
            
            logToUI("✅ Python execution finished.", 'result');

        } catch (err) {
            logToUI(`🔥 Python Error: ${err.message}`, 'error');
        }
    }
}